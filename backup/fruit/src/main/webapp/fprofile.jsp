...
<body>
    ...
    <div class="nav" style="margin-top:20px;">
        <ul>            
            <li><a href="Fdash.jsp">Add Products</a></li>
            ...
            <div class="container">
				<%
                        ...
                        try {
                            ...
                            pstmt = con.prepareStatement("SELECT f.firstName, f.lastName, f.phoneNumber, f.emailId, " +
                                    "l.address, l.city, l.state, l.country, l.pincode " +
                                    "FROM flogin fl " +
                                    "JOIN farmer_location_login fll ON fl.loginId = fll.loginId " +
                                    "JOIN farmer f ON fll.farmerId = f.farmerId " +
                                    "JOIN flocation l ON fll.locationId = l.locationId " +
                                    "WHERE fl.username = ?");
                            ...
                            if (rs.next()) {
                %>
                                <h1 align=center>Farmer Profile</h1>
                                <div class="profile">
                ...
                <%
                            ...
                            <%
                            	if(type!=null)
                            	{
                            		...
                            		{
                            			Class.forName("com.mysql.cj.jdbc.Driver");
                                        con = DriverManager.getConnection("jdbc:mysql://localhost:3306/backend", "root", "Ritesh@2903#");
                                        
                                        int farmerId = 0; // Default value if not found
                                        ...
                                        pstmt = con.prepareStatement("SELECT p.fruitname, p.quantity, p.pricepu " +
                                                "FROM product p " +
                                                "JOIN farmerproduct fp ON p.productId = fp.productId " +
                                                "WHERE fp.farmerId = ?");
                                        ...
                                        {
                                        	do{
                                        		out.println("<p>Product name: "+rs.getString("fruitname")+"<br>Quantity: "+rs.getInt("quantity")+"<br>Price per unit: "+rs.getInt("pricepu")+"</p>");
                                        	}while(rs.next());
                                        }
                                        else
                                        {
                                        	out.println("No product of the farmer");
                                        }
